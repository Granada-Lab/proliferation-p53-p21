...
for i in range(1):
    ...
    for ii in range(1,6):
        norm_median_p21_smoothed = wAn.sinc_smooth(dfs[ii].median(axis=1)/max(dfs[ii].median(axis=1)), T_c=40)
        ax.plot(time, norm_median_p21_smoothed, label='{} divisions'.format(ii), color=colors[ii], linewidth=5)
        if ii > 0:
            ax.axvline(x=np.median(times['time_{}div'.format(ii)]), linewidth=4, color=colors[ii])    
    ax.set_xlabel('Time [hours]')
    ax.set_ylabel('Normalized smoothed median p21 levels [a.u.]')
    ax.legend(loc='best')
