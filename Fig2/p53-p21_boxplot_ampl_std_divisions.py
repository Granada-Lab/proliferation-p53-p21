...
for i in range(4):
    ...
    # print('0 divisions:', len(df_list[0].dropna()))
    # print('1 divisions:', len(df_list[1].dropna()))
    # print('2 divisions:', len(df_list[2].dropna()))
    # print('3 divisions:', len(df_list[3].dropna()))
    # print('4 divisions:', len(df_list[4].dropna()))
    # # print('5 divisions:', len(df_list[5].dropna()))

    p_values_matrix = np.zeros((max_div, max_div))      
    fig=plt.figure(figsize = (12,10))
    sns.violinplot(data = df_list, showfliers = False, cut=0) #or df_list.vboxplot(showfliers = False)
    ...
    plt.text(max_div-3, high*0.75, r'Corr coeff: {:.2f} $\pm$ {:.2f}'.format(corr,se))
    plt.ylabel('p53 amplitude (a.u.)')
    plt.xlabel('Number divisions')
    plt.savefig(path2+'violinplot_ampl_p53_divisions_'+str(labels[i])+'.svg')
    plt.show() 
    ...
    plt.xlabel('Number divisions')
    plt.savefig(path2+'Violinplot_ampl_p21_divisions_'+str(labels[i])+'.svg')
    plt.show() 
    df_pvalues = pd.DataFrame(p_values_matrix)
