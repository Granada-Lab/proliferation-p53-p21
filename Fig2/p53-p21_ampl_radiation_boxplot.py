...
# print('0Gy:', len(df_ampl_p53[labels[0]].dropna()))
# print('2Gy:', len(df_ampl_p53[labels[1]].dropna()))
# print('4Gy:', len(df_ampl_p53[labels[2]].dropna()))
# print('10Gy:', len(df_ampl_p53[labels[3]].dropna()))

p_values_matrix = np.zeros((4, 4))   
fig = plt.figure(figsize = (12,10))
sns.violinplot(data = df_ampl_p53, showfliers = False, cut=0) #or df_ampl_p53.boxplot(showfliers = False)
...
plt.savefig(path2+'p53_violinplot_ampl_doses.svg')
plt.show()
...
p_values_matrix = np.zeros((4, 4))   
fig = plt.figure(figsize = (12,10))
sns.violinplot(data = df_ampl_p21, showfliers = False, cut=0) #or df_ampl_p21.boxplot(showfliers = False)
# Compute the t-test p-value
for jj in range(4):
    for ii in range(4):
        if ii > jj:
...
plt.savefig(path2+'p21_violinplot_ampl_doses.svg')
plt.show()
df_pvalues = pd.DataFrame(p_values_matrix)
